...
const setupTravelRoutes = (app) => {
    ...
    app.post(
        ...
        async (req, res) => {
            ...
            if (!email || !lessonUrl) {
                return res.status(400).json({error: 'Missing email or lessonUrl in request body'});
            }

            const body = `Здравствуйте!

Спасибо за ваш интерес к нашим урокам по путешествиям. Вот ссылка на бесплатный урок, который мы подготовили для вас:

${lessonUrl}

Мы надеемся, что этот урок будет полезным и вдохновляющим для ваших будущих путешествий. Если у вас возникнут вопросы или вы захотите узнать больше о наших курсах, не стесняйтесь обращаться к нам.

С наилучшими пожеланиями,
Команда «Путешествуй сам»`;

            await sendEmail(
                'Школа «Путешествуй сам» <svethappy3@gmail.com>',
                email,
                'Бесплатный урок по путешествиям',
                body
            );
