...
export const handleProduct = async (productId: string, customerEmail: string) => {
  ...
  try {
    ...
    if (userData) {
      ...
      console.log(`✓ Updated user ${customerEmail} with novice stream and tag in Firebase`);
    } else {
      // New user
      isNewUser = true;
      await userRef.set({
      ...
      console.log(`Added new user ${customerEmail} with novice stream and tag in Firebase`);
    }
